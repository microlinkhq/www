...
- NEVER: Add a crawler that builds a search index (`OAI-SearchBot`, `Claude-SearchBot`, `PerplexityBot`, `Amazonbot`…) to the user-agent redirect to markdown in `vercel.json`. The `.md` twins are `X-Robots-Tag: noindex`, so an indexer sent there drops the site from its results. That rule is for on-demand readers (`ChatGPT-User`, `Claude-User`) and training crawlers (`GPTBot`, `CCBot`); `test/markdown-routes.js` keeps the indexers on the HTML.
