...
6. `src/components/patterns/Aside/constants.js` if the guide set changes.
...
- Prefer stable demo URLs and selectors. Default to `microlink.io`, `github.com/microlinkhq`, or other durable targets.
...
- Do not document undocumented parameter shapes. If the type/examples do not show a boolean, object, string, or array form, do not invent it.
...
|------|----------|-----------------|
| `index.md` | Always | Quickstart, response model, canonical syntax, key decision points, roadmap |
...
| `caching-and-performance.md` | Freshness, cost, and speed matter | `meta`, `ttl`, `staleTtl`, `force`, optimization guidance |
| `private-pages.md` | Auth, sessions, forwarded headers, or endpoint choice matter | `headers`, `x-api-header-*`, `pro.microlink.io`, backend/proxy guidance |
| `troubleshooting.md` | Failure modes are common or multi-causal | timeouts, wrong output, auth mistakes, antibot, error-code paths |

Rename pages when the utility needs different language, but keep the workflow-first grouping.
...
5. Update shared navigation.
   - Update `src/content/docs/guides/index.md`.
