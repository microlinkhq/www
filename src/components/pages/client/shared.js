...
export const QUESTIONS = [
  {
    ...
    answer: (
      <>
        <div>
          The official universal HTTP client for the{' '}
          <Link href={DOCS_URL}>Microlink API</Link>. It organizes the API into
          semantic products — one method per product — so you call{' '}
          <code>microlink.screenshot(url)</code> instead of composing query
          strings by hand.
        </div>
      </>
    )
  ...
  {
    question: 'How is it different from the Microlink SDK?',
    answer: (
      <>
        <div>
          The <Link href='/integrations/sdk'>SDK</Link> is a UI component for
          rendering link previews in React, Vue, or vanilla JavaScript. The
          client is the programmatic interface: it returns data and assets from
          every Microlink product, with no UI attached.
        </div>
      </>
    )
  },
