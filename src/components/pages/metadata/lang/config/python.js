...
const PAGE_URL = 'https://microlink.io/metadata/python'
...
const python = {
  ...
  meta: {
    title: 'Python Metadata API — Extract Metadata from Any URL',
    description:
      'Extract title, description, image and logo from any URL in Python with a single HTTP request — no HTML parsing, no headless browser. Open Graph, Twitter Cards and JSON-LD merged. Free to start.',
    structured: [
      {
        '@context': 'https://schema.org',
        '@type': 'TechArticle',
        '@id': `${PAGE_URL}#article`,
        headline: 'Website Metadata API for Python',
        name: 'Website Metadata API for Python',
        description:
          'A developer guide to extracting website metadata programmatically in Python over the Microlink REST API — request, extract, framework integration, and link previews without parsing HTML or running a headless browser.',
        url: PAGE_URL,
        inLanguage: 'en',
        proficiencyLevel: 'Beginner',
        dependencies: 'Python 3.8+, standard library only',
        keywords:
          'python metadata api, website metadata api, url metadata python, open graph parser python, link preview api, url preview python',
        author: {
          '@type': 'Organization',
          name: 'Microlink',
      ...
      },
      {
        ...
        '@type': 'HowTo',
        '@id': `${PAGE_URL}#howto`,
        name: 'How to extract website metadata in Python',
        description:
          'Extract title, description, image and logo from any URL in Python with one HTTP request to the Microlink API in three steps.',
        tool: [
  ...
  hero: {
    title: (
      <>
        <Accent>Python</Accent> Metadata API
      </>
    ),
    subtitle:
  ...
  quickstart: {
    title: (
      <>
        Extract <Accent>metadata</Accent> in Python
      </>
    ),
    caption:
...
async def preview(url: str):
    async with httpx.AsyncClient() as client:
        res = await client.get(
            'https://api.microlink.io', params={'url': url}
        )
...
# Enrich a list of company URLs with metadata
def enrich(urls):
    rows = []
    for url in urls:
        ...
        with urllib.request.urlopen(
            f'https://api.microlink.io?{params}'
        ) as res:
            data = json.load(res)['data']
...
# python metadata.py https://microlink.io
