...
const PAGE_URL = 'https://microlink.io/metadata/php'
...
const php = {
  ...
  meta: {
    title: 'PHP Metadata API — Extract Metadata from Any URL',
    description:
      'Extract title, description, image and logo from any URL in PHP with a single HTTP request — no HTML parsing, no headless browser. Open Graph, Twitter Cards and JSON-LD merged. Free to start.',
    structured: [
      {
        '@context': 'https://schema.org',
        '@type': 'TechArticle',
        '@id': `${PAGE_URL}#article`,
        headline: 'Website Metadata API for PHP',
        name: 'Website Metadata API for PHP',
        description:
          'A developer guide to extracting website metadata programmatically in PHP over the Microlink REST API — request, extract, framework integration, and link previews without parsing HTML or running a headless browser.',
        url: PAGE_URL,
        inLanguage: 'en',
        proficiencyLevel: 'Beginner',
        dependencies: 'PHP 8.0+, cURL extension or file_get_contents',
        keywords:
          'php metadata api, website metadata api, url metadata php, open graph parser php, link preview api, url preview php',
        author: {
          '@type': 'Organization',
          name: 'Microlink',
      ...
      },
      {
        ...
        '@type': 'HowTo',
        '@id': `${PAGE_URL}#howto`,
        name: 'How to extract website metadata in PHP',
        description:
          'Extract title, description, image and logo from any URL in PHP with one HTTP request to the Microlink API in three steps.',
        tool: [
  ...
  hero: {
    title: (
      <>
        <Accent>PHP</Accent> Metadata API
      </>
    ),
    subtitle:
  ...
  quickstart: {
    title: (
      <>
        Extract <Accent>metadata</Accent> in PHP
      </>
    ),
    caption:
      'No package and no parser — the Microlink REST API turns any URL into normalized metadata with a single HTTP GET. Here it is with file_get_contents and the cURL extension that ship with PHP.',
    steps: [
      {
        title: 'Extract any URL',
        description:
          'A one-liner with file_get_contents — no Composer package to add. Point it at a page and read the metadata from the JSON response.',
        code: {
          language: 'php',
          title: 'extract.php',
          source: `<?php
$query = http_build_query(['url' => 'https://microlink.io']);
...
echo "<a href='{$data['url']}' class='card'>
  <img src='$image' alt='' />
  ...
  framework: {
    ...
    caption:
      'A Laravel route, a Symfony controller, or a WordPress shortcode — the same request becomes your own metadata endpoint for link previews and embeds.',
    examples: [
      {
        id: 'laravel',
        label: 'Laravel',
...
class PreviewController
{
    // GET /preview?url=https://microlink.io
    #[Route('/preview')]
    public function show(HttpClientInterface $client): JsonResponse
    {
      ...
      },
      {
        ...
        code: {
          ...
          title: 'functions.php',
          source: `<?php
// [microlink_card url="https://microlink.io"]
add_shortcode('microlink_card', function ($atts) {
    ...
    return "<a href='{$data['url']}' class='card'>
      <img src='$image' alt='' />
      <strong>{$data['title']}</strong>
    </a>";
});`
      ...
      },
      {
        ...
        code: {
          ...
          title: 'metadata.php',
          source: `<?php
// php metadata.php https://microlink.io
...
echo json_encode($res['data'], JSON_PRETTY_PRINT);`
  ...
  comparison: {
    ...
    caption:
      'Rolling your own means fetching HTML, parsing Open Graph and Twitter Cards with DOMDocument, merging JSON-LD and oEmbed, and adding Panther for JavaScript-injected tags. The API gives you normalized metadata from any page without any of the moving parts.',
    columns: [
      {
        tone: 'negative',
  ...
  tool: {
    ...
    caption:
      'Paste a URL and see the exact metadata response before you write a line of PHP.',
    cta: {
