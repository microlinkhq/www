...
const PAGE_URL = 'https://microlink.io/html/python'
...
const python = {
  ...
  meta: {
    title: 'Python HTML API — Get Rendered HTML from Any URL',
    description:
      'Get the fully rendered HTML of any URL in Python with a single HTTP request — real browser rendering, selector scoping, no headless browser to run. Free to start.',
    structured: [
      {
        '@context': 'https://schema.org',
        '@type': 'TechArticle',
        '@id': `${PAGE_URL}#article`,
        headline: 'HTML Extraction API for Python',
        name: 'HTML Extraction API for Python',
        description:
          'A developer guide to getting the rendered HTML of any URL programmatically in Python over the Microlink REST API — request, selector scoping, JavaScript rendering, and framework integration without running a headless browser.',
        url: PAGE_URL,
        inLanguage: 'en',
        proficiencyLevel: 'Beginner',
        dependencies: 'Python 3.8+, standard library only',
        keywords:
          'python html api, html api, get html from url python, render html api, scrape html python, website to html python',
        author: {
          '@type': 'Organization',
          name: 'Microlink',
      ...
      },
      {
        ...
        '@type': 'HowTo',
        '@id': `${PAGE_URL}#howto`,
        name: 'How to get the rendered HTML of any URL in Python',
        description:
          'Get the fully rendered HTML of any URL in Python with one HTTP request to the Microlink API in three steps.',
        tool: [
  ...
  quickstart: {
    title: (
      <>
        Get <Accent>HTML</Accent> in Python
      </>
    ),
    caption:
...
print(data['html'][:15])   # '<!DOCTYPE html>'`
        }
      },
      {
...
with urllib.request.urlopen(f'https://api.microlink.io?{params}') as res:
    html = res.read().decode()  # ready to serve or store`
        }
      }
    ]
  },
...
@app.get('/render')
def render(url: str):
    params = urllib.parse.urlencode({'url': url, 'data.html.attr': 'html'})
    with urllib.request.urlopen(f'https://api.microlink.io?{params}') as res:
        data = json.load(res)['data']
    ...
    return path`
      ...
      },
      {
        ...
        code: {
          language: 'python',
          title: 'html.py',
          source: `# python html.py https://microlink.io
import json
import sys
import urllib.parse
import urllib.request
