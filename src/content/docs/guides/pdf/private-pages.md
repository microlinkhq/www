---
title: 'Private pages'
description: 'Generate PDFs from pages behind logins, sessions, or header-based personalization. Learn when to use headers, how to pass sensitive credentials safely, and when to use the Pro endpoint.'
---
...
Many real PDF workflows involve private content: invoices, dashboards, internal documentation, staged pages, and user-specific views. The setup depends on whether the values you need to send are public or sensitive.
...
Use `headers` when the value is safe to expose, for example `Accept-Language`:
...
<Figcaption>Good for localization and request shaping. Avoid putting cookies or authorization tokens here because query parameters are public.</Figcaption>
...
Do not expose API keys, cookies, or authorization headers in client-side download or preview URLs.

- Keep authenticated PDF requests on your backend whenever possible.
...
This is especially important when you combine PDFs with `embed=pdf.url`, because the final URL can end up in markup, logs, browser devtools, or shared links.
...
For timeout errors, wrong endpoint issues, layout problems, or antibot failures such as `EPROXYNEEDED`, continue with [troubleshooting](/docs/guides/pdf/troubleshooting).
