...
Use [attr](/docs/sdk/methods/extract/attr) with `text` when your consumer needs readable plain text instead of Markdown structure or HTML markup. Point [url](/docs/api/parameters/url) at any source — a web page, a PDF file, or an office document — and Microlink returns the content as a string.
...
For HTML pages, use a [selector](/docs/sdk/methods/extract/selector) when the page has navigation, footers, or other text that should not be included:
