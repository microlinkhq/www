...
Use [attr](/docs/sdk/methods/extract/attr) with `json` when the target URL is already a JSON endpoint. Microlink fetches the URL, parses the full response body with `JSON.parse`, and returns native structured data instead of a string.
...
`attr: 'json'` is whole-page only. Do not combine it with [selector](/docs/sdk/methods/extract/selector) or [selectorAll](/docs/sdk/methods/extract/selectorAll); Microlink parses the entire endpoint response.
