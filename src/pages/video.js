...
import {
  ...
  FAQ_ITEMS,
  META
} from 'components/pages/video/shared'

const FEATURES = [
  {
    title: 'Direct File URLs',
    description:
      'The response carries a playable media URL rather than an iframe, so you can render it in a video element or queue it for processing.'
  },
  {
  ...
  },
  {
    ...
    description:
      'Social posts, video hosts, news sites and blogs all return the same response shape, replacing a bespoke integration per provider.'
  },
  {
  ...
  },
  {
    ...
    description:
      'Request title, description and poster image alongside the video and pay for a single render instead of several.'
  },
  {
...
const REPOS = ['youtube-dl-exec', 'ffprobe', 'metascraper']
...
const VideoPage = () => (
  <Layout>
    ...
    <OpenSource
      repos={REPOS}
      accent={ACCENT}
      caption='The Microlink media pipeline is powered by battle-tested open source libraries used by thousands of developers worldwide. Our video extraction API is built on an open source foundation: explore the code, contribute, or run it yourself.'
    />
    <Features
      ...
      caption={
        <>
          ...
          the <Link href='/docs/api/parameters/video'>video API</Link>.
        </>
      }
      features={FEATURES}
    />
