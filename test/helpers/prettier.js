...
test('js', async () => {
  ...
  const output = await prettier(code, 'js')
...
test('json', async () => {
  ...
  const output = await prettier(code, 'json')

  expect(output).toMatchSnapshot()
})

test('headers', async () => {
  const code = `content-type: application/json
cache-control: public, max-age=31536000
x-custom-header: some-value`

  const output = await prettier(code, 'headers')
